...
class ShowImporter
{
    ...
    public function import(string $movieName, string $hallName, string $day, string $month, string $year, string $hour, string $minutes)
    {
        ...
        $this->showRepository->insert(new Show(1, $movie->getItem(), $hall->getItem(), $date));


    }


    private function formatDate(string $day, string $month, string $year, string $hour, string $minutes) : string
    {
        $dateString = '';
        $dateString .= $year . '-';
